  ...
  try {
    const result = await db.query(
      `SELECT
       ...
       -- 週の範囲 [monthStart, monthStart + 1 month) で貸出を突き合わせる
       LEFT JOIN "Loan" l
         ON l."loanedAt" >= w."monthStart"
        AND l."loanedAt" < w."monthStart" + interval '1 month'
       GROUP BY w."monthStart"
       ORDER BY w."monthStart" ASC`,
      [anchorDate]
    );

    return NextResponse.json(result.rows, { status: 200 });
  } catch (error) {
    console.error("月次統計の取得に失敗:", error);
    return NextResponse.json(
      { error: "月次統計の取得に失敗しました" },
      { status: 500 }
    );
  }
}
